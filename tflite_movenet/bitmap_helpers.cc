  ...
  for (int i = 0; i < height; i++) {
    ...
    for (int j = 0; j < width; j++) {
      ...
      case 3:
          ...
          //std::cout << "BGR\t" << unsigned(input[src_pos]) << "\t" << unsigned(input[src_pos+ 1]) << "\t" << unsigned(input[src_pos + 2]) << std::endl;
          //std::cout << "RGB\t" << unsigned(output[dst_pos]) << "\t" << unsigned(output[dst_pos + 1]) << "\t" << unsigned(output[dst_pos + 2]) << std::endl;
          break;
        case 4:
          // BGRA -> RGBA
          output[dst_pos] = input[src_pos + 2];
...
std::vector<uint8_t> read_bmp(const std::string& input_bmp_name, int* width,
  ...
  if (!file) {
    std::cout << "input file " << input_bmp_name << " not found";
    exit(-1);
  }
